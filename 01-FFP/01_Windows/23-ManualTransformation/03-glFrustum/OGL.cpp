...
void resize(int width, int height)
{
	...
	h = tan(((fov / 2) / 180) * M_PI) * n_pers;
	w = h * ((GLfloat)width / (GLfloat)height);
	...
	//initialization of Perspective Matrix for Frustum
	...
	perspectiveMatrix[5] = (2*n) / (t - b);
	perspectiveMatrix[6] = 0.0f;
	perspectiveMatrix[7] = 0.0f;
	perspectiveMatrix[8] = ((r + l) / (r - l));
	perspectiveMatrix[9] = ((t + b) / (t - b));
	perspectiveMatrix[10] = -((f + n) / (f - n));
	perspectiveMatrix[11] = -1.0f;
	perspectiveMatrix[12] = 0.0f;
	perspectiveMatrix[13] = 0.0f;
	perspectiveMatrix[14] = -((2 * f * n) / (f - n));
